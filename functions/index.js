...
const ONESIGNAL_API_KEY = "os_v2_app_nlbikmjdfjb2vkla2eiuzk3mrktlzfkhs45uv5nfxgmk4qa6bcaih57lmaglff3ijbmekrcyoy7zx7nc24repkicstco3wct5bzxyey"; // <-- ĐIỀN KEY VÀO ĐÂY
...
exports.checkDeadlines = onSchedule("every 15 minutes", async (event) => {
    ...
    console.log("Bắt đầu kiểm tra deadline... Thời gian hiện tại:", new Date(now).toISOString());
    ...
    try {
                ...
                Object.keys(userData.personal_tasks).forEach(taskId => {
                    ...
                    if (shouldRemind(task, now, oneHourLater)) {
                        ...
                        if (userData.email && GMAIL_USER) {
                            emails.push({ to: userData.email, subject: "⏰ Deadline Task Cá Nhân", msg });
                        }
                    ...
                    // Nhắc 1 ngày trước hạn
                        ...
                        if (userData.email && GMAIL_USER) {
                            emails.push({ to: userData.email, subject: "📅 Nhắc nhở Task Cá Nhân", msg });
                        }
                    }
                });
            }
                    ...
                    if (assigneeData) {
                        ...
                        if (shouldRemind(task, now, oneHourLater)) {
                            ...
                            if (assigneeData.email && GMAIL_USER) {
                                emails.push({ to: assigneeData.email, subject: "⏰ Deadline Task Dự Án", msg });
                            }
                        ...
                        // Nhắc 1 ngày trước hạn
                            ...
                            notifications.push({ externalId: assigneeId, msg });
                            if (assigneeData.email && GMAIL_USER) {
                                emails.push({ to: assigneeData.email, subject: "📅 Nhắc nhở Task Dự Án", msg });
                            }
                        }
                    }
                }
        ...
        // Gửi email nếu có
        const mailPromises = emails.map(e =>
            sendMail(e.to, e.subject, e.msg)
        );

        await Promise.all([...pushPromises, ...mailPromises]);
        console.log(`Hoàn tất: Đã gửi ${notifications.length} push notification và ${emails.length} email.`);
